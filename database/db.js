...
console.log('::::::'+'mongodb://'+config.get('api.mongodb_host')+':'+config.get('api.mongodb_port')+'/'+config.get('api.mongodb_db'));
mongoose.connect('mongodb://'+config.get('api.mongodb_host')+':'+config.get('api.mongodb_port')+'/'+config.get('api.mongodb_db'),{ useNewUrlParser: true });
...
var DB = function () {
    this.mongoClient = {};
    var filename = path.join(path.dirname(__dirname).replace('app', ''), './database/table.json');
